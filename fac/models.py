...
class Cliente(ClaseModelo):
    ...
    apellidos = models.CharField('Apeliidos', max_length=100)
    celular = models.CharField('Celular', max_length=20, null=True, blank=True)
    tipo = models.CharField('Tipo', max_length=10, choices=TIPPO_CLIENTE, default=NAT)
...
class FacturaEnc(ClaseModelo2):
    ...
    class Meta:
        ...
        permissions = [
            ('sup_caja_facturaenc', 'Permiso de supervisor de caja encabezado')
        ]
...
def detalle_fac_guardar(sender, instance, **kwargs):
    ...
    pelicula_id = instance.pelicula.id
    ...
    if enc:
        ...
        descuento = FacturaDet.objects \
            .filter(factura=factura_id) \
            .aggregate(sub_total=Sum('descuento')) \
            .get('descuento', 0.00)
    ...
    prod = Pelicula.objects.filter(pk=pelicula_id).first()
    if prod:
